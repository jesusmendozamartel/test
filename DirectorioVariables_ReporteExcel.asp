...
Response.Charset= "ISO-8859-1" 
	...
	NivelText=Request.QueryString("NivelText")

	anio1=anio-1

	SQL="exec sp_SMVMA_BALGEN_DirectorioVariables_AnioMoneda "&anio&","&moneda&","&nivel
	'response.Write(sql)
	'response.End()
	...
	Agrupacion="Información mostrada a nivel "&NivelText

	MonedaText=""
	if moneda="Total" then
		MonedaText="Empresas que declaran en Soles y Dolares / Información mostrada en miles de Soles (Incluye Dólares convertidos a Soles)"
	else
		MonedaText="Empresas que declaran en " &moneda&" / Información mostrada en miles de " &moneda
	end if
	...
	Response.Charset = "ISO-8859-1"
	Response.Write("<table ><tr><td colspan='20' align='center'  style=""font-family:Arial, Helvetica, sans-serif; font-size:20px; color:#003300"">"&Titulo&"</td></tr><tr><td colspan='20' align='center'  style=""font-family:Arial, Helvetica, sans-serif; font-size:10px; color:#003300""><strong>"&MonedaText&"</strong></td></tr><tr><td colspan='20' align='center'  style=""font-family:Arial, Helvetica, sans-serif; font-size:10px; color:#003300"">"&Agrupacion&"</td></tr>")
	...
	For intLoop = i To (objFieldsC.Count - 1)
	    response.write("<td bgcolor='#D5D9DC' align='center'><strong>"&CDbl(objFieldsC.Item(intLoop).Name)&"</strong></td>")
    Next
    response.write("</tr>")
    ...
    while not rs.eof
	    For intLoop = 0 To (objFieldsC.Count - 1)
			...
			if (nivel=1 and intLoop>5) or (nivel<>1 and intLoop>1) then
		    	response.write("<td align='right'>"&rs(intLoop)&"</td>")
		    else
	    		response.write("<td STYLE='vnd.ms-excel.numberformat:@' align='left'>"&rs(intLoop)&"</td>")
			end if
	    Next      	
    	response.write("</tr>")
		rs.MoveNext
